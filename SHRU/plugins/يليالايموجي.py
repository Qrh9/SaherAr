...
async def search_accounts(event):
    namee = event.pattern_match.group(1).strip()
    if not namee:
    ...
    try:
        ...
        if not profiles:
            return await edit_or_reply(event, f"⌔∮ لم يتم العثور على حسابات متعلقة بهذا الاسم {namee}.")
        
        await edit_or_reply(event, f"⌔∮ الحسابات المتعلقة بالاسم {namee}:\n{profiles}")
